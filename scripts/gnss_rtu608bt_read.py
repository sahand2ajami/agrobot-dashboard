#!/usr/bin/env python3
"""
GeoAstra RTU608BT GNSS reader.
Supports GPS + BeiDou + GLONASS + Galileo via NMEA 0183 over USB or Bluetooth.
Parses NMEA sentences, prints a live terminal display,
and writes /tmp/gnss_coords.json for the map viewer.

USB: auto-detected on /dev/ttyUSB* or /dev/ttyACM*.
Bluetooth: pair the device (BT name "Geoastra", PIN 1234), bind to /dev/rfcomm0
    with:  sudo rfcomm bind 0 <MAC>
    then symlink:  sudo ln -sf /dev/rfcomm0 /dev/gnss  (optional, for auto-detect)
"""
import json
import os
import sys
...
LOG_DIR_DEFAULT = os.path.expanduser("~/dual-robot-dashboard/logs/gnss")
# Device baud rate is 38400; try common alternatives for robustness.
BAUD_RATES = [38400, 9600, 4800, 115200]
CANDIDATE_PORTS = [
    "/dev/gnss",       # stable udev symlink (preferred)
    "/dev/ttyUSB0", "/dev/ttyUSB1",
    "/dev/ttyACM0", "/dev/ttyACM1",
    "/dev/rfcomm0", "/dev/rfcomm1",   # Bluetooth (after rfcomm bind)
]
...
def render(state, port, baud, log_path):
    ...
    print(f"  GeoAstra RTU608BT  |  {port}  |  {baud} baud  |  {ts}")
    print(f"  {'─' * 50}")
...
def main():
    ...
    if not port:
        print("ERROR: No GNSS device found.")
        print("  USB : plug in the GeoAstra RTU608BT via USB.")
        print("  BT  : pair (name 'Geoastra', PIN 1234), bind with")
        print("        sudo rfcomm bind 0 <MAC>  then re-run.")
        print("  Or specify the port explicitly: gnss_rtu608bt_read.py /dev/ttyUSB0")
        sys.exit(1)
